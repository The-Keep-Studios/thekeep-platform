...
require_var() {
  ...
  if [ -z "${!name:-}" ]; then
    ...
    echo "Create scripts/production.env from scripts/production.env.example"
    exit 1
  fi
}
...
WISEMAPPING_URL="${WISEMAPPING_URL:-https://mindmaps.thekeepstudios.com}"
MONITORING_ENABLED="${MONITORING_ENABLED:-true}"
GRAFANA_HOST="${GRAFANA_HOST:-grafana.thekeepstudios.com}"
PROMETHEUS_HOST="${PROMETHEUS_HOST:-prometheus.thekeepstudios.com}"
ALERTMANAGER_HOST="${ALERTMANAGER_HOST:-alerts.thekeepstudios.com}"
GRAFANA_URL="${GRAFANA_URL:-https://${GRAFANA_HOST}}"
PROMETHEUS_URL="${PROMETHEUS_URL:-https://${PROMETHEUS_HOST}}"
ALERTMANAGER_URL="${ALERTMANAGER_URL:-https://${ALERTMANAGER_HOST}}"
...
khelm() {
  sudo -u k3s-admin -H env KUBECONFIG=/home/k3s-admin/.kube/config helm "$@"
}
...
echo "Applying leantime auth secret material..."
...
if [ "${MONITORING_ENABLED}" = "true" ]; then
  echo "Applying monitoring secret material..."
...
if [ -n "${CLOUDFLARE_TUNNEL_TOKEN:-}" ]; then
  echo "Applying optional cloudflare tunnel token..."
...
kadmin apply -f kubernetes/platform/authentik/standalone.yaml

if [ "${MONITORING_ENABLED}" = "true" ]; then
  echo "Deploying monitoring stack (Prometheus + Grafana + Loki)..."
  khelm repo add prometheus-community https://prometheus-community.github.io/helm-charts >/dev/null 2>&1 || true
  khelm repo add grafana https://grafana.github.io/helm-charts >/dev/null 2>&1 || true
  khelm repo update

  khelm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack \
    ...
    --set-string alertmanager.ingress.hosts[0]="${ALERTMANAGER_HOST}" \
    --wait --timeout 20m

  khelm upgrade --install loki grafana/loki-stack \
    --namespace monitoring \
    --values kubernetes/platform/monitoring/loki-stack.values.yaml \
    --wait --timeout 20m
fi
...
kadmin apply -f kubernetes/apps/leantime/production.yaml
kadmin apply -f kubernetes/apps/wisemapping/production.yaml
kadmin apply -f kubernetes/apps/gitlab/standalone.yaml

echo "Waiting for platform readiness..."
kadmin wait --for=condition=available --timeout=600s deployment/authentik-postgres -n identity
kadmin wait --for=condition=available --timeout=600s deployment/authentik-redis -n identity
kadmin wait --for=condition=available --timeout=900s deployment/authentik-server -n identity
kadmin wait --for=condition=available --timeout=900s deployment/authentik-worker -n identity

echo "Waiting for app readiness..."
kadmin wait --for=condition=available --timeout=600s deployment/leantime-mariadb
kadmin wait --for=condition=available --timeout=900s deployment/leantime
kadmin wait --for=condition=available --timeout=600s deployment/wisemapping-postgres -n wisemapping
kadmin wait --for=condition=available --timeout=900s deployment/wisemapping -n wisemapping
kadmin wait --for=condition=available --timeout=1800s deployment/gitlab -n gitlab

echo ""
echo "===================================================="
echo "Production-like baseline deployed."
echo "Auth hub: ${AUTHENTIK_URL}"
echo "Leantime: ${LEANTIME_URL}"
echo "GitLab: ${GITLAB_URL}"
echo "Wisemapping: ${WISEMAPPING_URL}"
if [ "${MONITORING_ENABLED}" = "true" ]; then
...
echo ""
echo "OIDC status:"
echo "  Leantime enabled: ${LEAN_OIDC_ENABLE}"
echo "  GitLab enabled:   ${OIDC_ENABLED}"
echo "  Wisemapping enabled: ${WISEMAPPING_OAUTH_ENABLED}"
echo "Monitoring status:"
echo "  Enabled: ${MONITORING_ENABLED}"
echo ""
echo "If OIDC is still disabled, finish Authentik provider setup first,"
echo "update scripts/production.env, and re-run ./bootstrap-production.sh."
echo "===================================================="
